...
export default function ModuleManagementPanel() {
  ...
  const { modules, seedData, deleteModule, updateModuleOrder, saveModule } =
    useAdminContent();
                    ...
                    {monthModules.length === 0 ? (
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
                        {monthModules.map((mod, modIdx) => (
                          <div
                            key={mod.id}
                            className="bg-brand-surface border border-white/10 p-6 flex flex-col justify-between group"
                          >
                            <div>
                              ...
                              <h3 className="font-black uppercase tracking-tight text-lg mb-2">
                                {mod.title}
                              </h3>
                              <p className="text-[10px] text-white/40 uppercase tracking-widest line-clamp-2">
                                {mod.description}
                              </p>
                            </div>
                                ...
                                <button
                                  ...
                                  onClick={() =>
                                    updateModuleOrder(
                                      mod.id,
                                      mod.order,
                                      monthModules[modIdx - 1].id,
                                      monthModules[modIdx - 1].order,
                                    )
                                  }
                                  className="p-1 text-white/20 hover:text-brand-primary disabled:opacity-0 transition-colors"
                                >
                                  <ChevronUp size={16} />
                                </button>
                                <button
