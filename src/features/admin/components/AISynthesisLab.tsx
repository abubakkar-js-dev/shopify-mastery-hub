...
export default function AISynthesisLab({ onSuccess }: AISynthesisLabProps) {
  ...
  const [status, setStatus] = useState<
    "idle" | "analyzing" | "synthesizing" | "securing" | "error" | "preview"
  >("idle");
  ...
  const generateAllChunks = async () => {
    ...
    try {
      ...
      for (let week = 1; week <= TOTAL_WEEKS; week++) {
        ...
        const existingLessonsSnap = await getDocs(
          ...
          <div className="flex-1">
            <div className="flex items-center gap-3 mb-2">
              ...
              <div className="flex items-center gap-2 group/hint relative">
                <h2 className="text-xl font-black uppercase tracking-tighter italic text-brand-primary">
                  AI Synthesis Lab
                </h2>
                <RiInformationLine
                  className="text-white/20 hover:text-brand-primary transition-colors cursor-help"
                  size={16}
                />
                ...
                <div className="absolute left-full ml-4 top-0 w-64 p-4 bg-[#1a1a1a] border border-white/10 rounded-lg opacity-0 invisible group-hover/hint:opacity-100 group-hover/hint:visible transition-all z-50 pointer-events-none">
                  <p className="text-[10px] text-white/60 leading-relaxed font-medium uppercase tracking-wider">
                    The Syllabus Engine uses a{" "}
                    <span className="text-brand-primary">Soft-Merge</span>{" "}
                    protocol. It identifies empty modules and lessons to
                    populate them with high-performance content while{" "}
                    <span className="text-white font-bold">
                      preserving your existing YouTube URLs and custom data.
                    </span>
                  </p>
                </div>
              </div>
            </div>
            <p className="text-[10px] text-white/30 font-bold uppercase tracking-[0.3em] mb-6">
              Generate a complete 12-week high-performance roadmap instantly
            </p>
                ...
                >
                  <div className="mb-6">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        updateModule(chunkIndex, "title", e.target.value)
                      }
                    ...
                    <textarea
                      ...
                      onChange={(e) =>
                        updateModule(chunkIndex, "description", e.target.value)
                      }
        ...
        {status === "error" && (
          <div className="mt-6 p-4 bg-red-500/5 border border-red-500/10 flex items-center gap-3 text-red-500">
            ...
            <span className="text-[10px] font-bold uppercase tracking-widest">
              Neural Link Distorted. Check logs and retry.
            </span>
          </div>
        )}
      </div>
