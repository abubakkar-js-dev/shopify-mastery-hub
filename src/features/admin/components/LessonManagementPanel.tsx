  ...
  return (
    ...
    >
      <header className="flex justify-between items-end">
        ...
        <button
          onClick={() => {
            const nextOrder = filteredLessons.length > 0 ? Math.max(...filteredLessons.map(l => l.order)) + 1 : 1;
            setIsEditingLesson({ 
              id: `lesson-${Date.now()}`, 
        ...
        <div className="w-full md:w-64 space-y-4">
          ...
          <div className="bg-brand-surface border border-white/10 p-4">
            <div className="flex items-center gap-2 mb-4 text-white/40">
              <Filter size={12} />
              <span className="text-[10px] font-black uppercase tracking-widest">Filter by Module</span>
            </div>
          ...
          <div className="flex items-center justify-between bg-white/2 p-4 border border-white/10">
            <div className="flex items-center gap-3">
              <span className="text-[10px] font-mono text-brand-primary font-bold">
                {selectedModuleId ? `Module: ${currentModule?.title}` : "All Modules"}
              </span>
              <span className="text-white/20">•</span>
              <span className="text-[10px] text-white/40 uppercase tracking-widest">{filteredLessons.length} Sessions Found</span>
            </div>
          </div>

          <div className="space-y-3">
            {filteredLessons.map((lesson, lessonIdx) => (
              <div key={lesson.id} className="flex items-center justify-between p-4 bg-brand-surface border border-white/10 hover:border-brand-primary/20 transition-colors group/lesson">
                <div className="flex items-center gap-4">
                  <div className="flex flex-col">
                    <button 
                      disabled={lessonIdx === 0} 
                      onClick={() => updateLessonOrder(lesson.moduleId, lesson.id, lesson.order, filteredLessons[lessonIdx-1].id, filteredLessons[lessonIdx-1].order)} 
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-3 mt-1">
                      ...
                      <span className="text-[9px] text-white/20 uppercase tracking-widest">
                        {lesson.videos.length} Videos • {lesson.tasks?.length || 0} Tasks
                      </span>
                      <span className="text-[9px] text-white/20 uppercase tracking-widest">
                ...
                <div className="flex items-center gap-1">
                  <button onClick={() => setIsEditingLesson(lesson)} className="p-2 text-white/20 hover:text-white transition-colors"><Edit3 size={16} /></button>
                  <button onClick={() => deleteLesson(lesson.moduleId, lesson.id)} className="p-2 text-white/20 hover:text-red-400 transition-all"><Trash2 size={16} /></button>
                </div>
              </div>
            ))}
            ...
            {filteredLessons.length === 0 && (
              <div className="p-12 border border-dashed border-white/5 text-center">
                <p className="text-[10px] text-white/20 uppercase tracking-widest italic">No sessions found in this view.</p>
              </div>
            )}
          </div>
        </div>
